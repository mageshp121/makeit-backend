...
export default (dependecies:any)=>{
    const  getAllLessoneByCourseId = async (req:Request,res:Response)=>{
        console.log(req.params,'<= params - course Id =>');
        const courseId  = req.params.id
        const { useCase: {  getLessoneByCourseId_UseCase },} = dependecies;
        const { exicutefuncation  } = await getLessoneByCourseId_UseCase(dependecies)
