...
export const createUser_useCase = async (dependdecies: any) => {
  ...
  if (!userRepository)console.log("have no repository");
   const exicutefunction = async ({firstName,lastName,email,phone,roll,password,}: userData) => {
    ...
    if (!userExist) {
      // Hashing password
       ...
       // Creating new entity
      let user:object =  new userProfile({firstName,lastName,email,phone,password:hashPassword,roll,isOtPVerified:false});
